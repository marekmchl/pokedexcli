...
func GetPokemon(name string) (*Pokemon, error) {
	...
	res, err := http.Get(url)
	if err != nil {
		return &Pokemon{}, fmt.Errorf("failed: pokapi.go - GetPokemonBaseExperience - get: %v", err)
	}
	defer res.Body.Close()

	rawJson, err := io.ReadAll(res.Body)
	if err != nil {
		return &Pokemon{}, fmt.Errorf("failed: pokapi.go - GetPokemonBaseExperience - read: %v", err)
	}

	pokemonMap := &Pokemon{}
	if err := json.Unmarshal(rawJson, pokemonMap); err != nil {
		return &Pokemon{}, fmt.Errorf("failed: pokapi.go - GetPokemonBaseExperience - unmarshal: %v", err)
	}
